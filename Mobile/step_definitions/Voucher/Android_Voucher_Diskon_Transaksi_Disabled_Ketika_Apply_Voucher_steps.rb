Given(/^user berada di homepage dan sudah memilih produk pelanggan dan voucher #diskondisabled$/) do 
...
When(/^user klik titik 3 #diskondisabled$/) do
    @app.poinvoucherpromo.btn_titiktiga1.click
    sleep(1)
end

Then(/^button diskon disabled #diskondisabled$/) do
    @app.promo.btn_diskon.click
    begin
        if waitFor {@app.promo.btn_simpan.displayed?}
            p 'scenario gagal, button diskon transaksi bisa di click'
        end
    rescue
        p 'scenario berhasil'
    end
